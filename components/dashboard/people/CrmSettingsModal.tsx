...
import { LearnVoiceModal } from "./LearnVoiceModal";
...
export function CrmSettingsModal({ db, live, setState, onClose }: {
  ...
  return (
    <>
        ...
        <div className={sectionCls}>
          ...
          <div className="mt-4 border-t border-stone-100 pt-3">
            ...
            <div className={noteCls}>
              Filter and hand-pick specific sent emails to learn your tone. The text of the emails you pick goes to Claude only for that step and is not stored — only the summary is kept.
            </div>
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2.5">
              <button type="button" onClick={() => setShowLearn(true)} className={btnAlt} style={mono}>
                Learn my voice from sent mail…
              </button>
              {db.settings.voice?.styleSummary && (
                <button type="button" onClick={learnClear} className={btnGhost} style={mono}>
                  Clear learned voice
                </button>
              )}
            </div>
      ...
      {showLearn && (
        <LearnVoiceModal db={db} setState={setState} onClose={() => setShowLearn(false)} />
      )}
      {showTiers && (

...
// running timer; in React we instead force a re-render once per second (via a
// dummy state bump) so any live `taskTime(...)` reads in the tree pick up fresh
// wall-clock time. The interval is armed only while some task has `timerStart !=

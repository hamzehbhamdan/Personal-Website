...
/**
 * Presentational kanban card. Drag + click are wired by the wrapper in
 * BoardPanel. Spans-only (no div/p): the wrapper nests this inside a native
 * <button>, whose content model is phrasing content. `pr-4` on the header row
 * keeps the label and running-dot clear of the wrapper's corner drag handle.
 */
export function BoardCard({ db, task, accent }: { db: CoachDB; task: Task; accent: string }) {
  ...
  return (
    <span
      className="relative block rounded-[10px] border border-stone-200 bg-white p-3 pl-3.5 hover:border-stone-300"
    ...
    >
      ...
      <span className="flex items-start justify-between gap-2 pr-4">
        <span className="text-[13px] leading-snug text-stone-800">{task.label}</span>
        {running && (
      ...
      {subTotal > 0 && (
        <span className="mt-2 block">
          <span className="block h-1 overflow-hidden rounded-full bg-stone-100">
            <span className="block h-full rounded-full bg-[#A51C30] transition-[width]" style={{ width: `${pct}%` }} />
          </span>

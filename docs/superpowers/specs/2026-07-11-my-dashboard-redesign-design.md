...
**Residual risks — Netlify secret-scanning config (adversarially audited 2026-07-15).** Netlify's secret scanner inspects repo code + build output for the *values* of every env var, so `netlify.toml` omits four **non-secret** KEYS (`NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, `ALLOWED_EMAIL`, `GOOGLE_OAUTH_REDIRECT`) and the `public/` PATH. The real secrets (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `TOKEN_ENC_KEY`, `GCAL_CLIENT_SECRET`) are deliberately **not** omitted, so a genuine leak still fails the build; `.next` is **not** omitted, so a server-secret bundle-inline is still caught. A three-way adversarial audit confirmed no current exposure and flagged two residual (future-only) risks:

- **The anon-key omission is contingent on RLS staying strict.** `NEXT_PUBLIC_SUPABASE_ANON_KEY` ships to the browser by design (decoded: `role=anon`, *not* `service_role`) and is non-secret **only because** strict per-owner RLS blocks it — `auth.uid() = user_id` with `WITH CHECK` on every table, `match_documents` `SECURITY INVOKER` + owner-filtered + `revoke … from anon, public`, no nil-UUID/`using(true)`/demo backdoor (verified live in A0). If RLS were ever disabled or a permissive/`service_role` policy added, the omitted public key would immediately become a full data read/write exposure that the scanner is no longer watching. **Operational invariant: RLS must stay strict on every table.** Recommended hardening: a deploy/CI assertion that an unauthenticated anon-key `SELECT` against each table returns zero rows.
- **`public/` is a broad path omission.** The whole `public/` tree (served world-readable) is skipped to suppress the `public/playground/mcp-injection-lab` decoy tokens (`AKIA…CANARY9`, `ghp_canaryToken…`, base64 pagination cursors — teaching artifacts, not real secrets). A real secret manually dropped anywhere under `public/` would therefore ship un-flagged. None is there today and the Next build never writes env values into `public/` (only into the still-scanned `.next`), so this is a defense-in-depth gap for a future manual mistake, not a current exposure. Recommended hardening: narrow `SECRETS_SCAN_OMIT_PATHS` to `public/playground/mcp-injection-lab` so the rest of `public/` stays scanned (test against Netlify's scan first).
